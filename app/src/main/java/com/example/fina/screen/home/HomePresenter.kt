...
class HomePresenter internal constructor(private val mCoinRepository: CoinRepository?) : HomeContract.Presenter {
    ...
    fun updateParamsWithCurrency(currency: Currency) {
        val timePeriodOption = this.currentParams.getTimePeriodOption()
        this.currentParams = ExtraParams(currency.uuid, timePeriodOption)
        getCoins()
    }

    fun updateParamsWithTimePeriod(timePeriodOption: TimePeriodOption) {
        val currency = this.currentParams.getReferenceCurrencyUuid()
        this.currentParams = ExtraParams(currency, timePeriodOption)
        getCoins()
    }
